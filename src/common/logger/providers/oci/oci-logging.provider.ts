...
export class OCILoggingProvider implements CloudLoggingProvider {
    ...
    private async initializeClient(): Promise<void> {
        ...
        this.initializationPromise = (async () => {
            try {
                ...
                try {
                    const builder =
                        new (common as any).InstancePrincipalsAuthenticationDetailsProviderBuilder();
                ...
                (this.loggingClient as { regionId?: string }).regionId = this.config.region;
            } catch (error) {
    ...
    private removeOracleFields(obj: any): any {
        ...
        if (Array.isArray(obj)) return obj.map((item) => this.removeOracleFields(item));
    ...
    private normalizeDates(obj: any): any {
        ...
        if (typeof obj === 'object') {
            ...
            for (const [key, value] of Object.entries(obj)) {
                ...
                } else if (key.toLowerCase() === 'datetime' && typeof value === 'number') {
                    normalized[key] = new Date(value).toISOString();
                } else if (key.toLowerCase() === 'timestamp' && typeof value === 'number') {
                    normalized[key] = new Date(value).toISOString();
                } else {

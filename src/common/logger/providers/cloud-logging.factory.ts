...
export class CloudLoggingFactory {
    ...
    private static createOCIProvider(
        ...
        const logGroupOcid = configService.get<string>('OCI_LOGGING_LOG_GROUP_OCID');
        ...
        if (!logGroupOcid) {
            throw new Error('OCI_LOGGING_LOG_GROUP_OCID is required when CLOUD_LOGGING_PROVIDER=oci');
        }
        ...
        if (!logId) {
            throw new Error('OCI_LOGGING_LOG_ID is required when CLOUD_LOGGING_PROVIDER=oci');
        }
        ...
        const config: OCILoggingConfig = {
            ...
            logId,
            region,
            compartmentOcid: configService.get<string>('OCI_LOGGING_COMPARTMENT_OCID'),

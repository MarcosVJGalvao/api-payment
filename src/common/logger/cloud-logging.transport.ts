...
import { CloudLoggingProvider, CloudLogEntry } from './providers/cloud-logging.provider';
...
export interface CloudLoggingTransportOptions extends TransportStream.TransportStreamOptions {
...
export class CloudLoggingTransport extends TransportStream {
    ...
    private shouldSkipMessage(message: string | undefined, context: string | undefined): boolean {
        if (context === 'Bootstrap' || (context && context.toLowerCase().includes('bootstrap'))) {
            return true;
        }
    ...
    private cleanLogEntry(logEntry: Record<string, any>): Partial<CloudLogEntry> {
        ...
        const skipKeys = ['splat', 'level', 'timestamp', 'message', 'skipCloud', 'skipOci', 'oracle', 'context', 'stack'];
    ...
    log(info: winston.LogEntry, callback: () => void): void {
        ...
        const shouldSkipCloud = (info.skipCloud as boolean) || (info.skipOci as boolean);
        ...
        const moduleName = getModuleName(context, info as Record<string, any>, info.stack as string);
        ...
        const logEntry: CloudLogEntry = {
            ...
            level: this.cleanLevel((info.level as string) || 'info'),
            message: message || '',
        };

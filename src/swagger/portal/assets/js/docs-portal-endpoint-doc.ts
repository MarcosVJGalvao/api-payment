(function initDocsPortalEndpointDocModule(globalScope) {
    function createEndpointDocModule(deps) {
        const { escapeHtml, marked, getOpenApiSpec, getAuthLabelForEndpoint, renderRequiredHeadersBox, } = deps;
        function resolveSchema(schema) {
            ...
            if (schema.$ref) {
                ...
                return ((spec && spec.components && spec.components.schemas && spec.components.schemas[refPath]) ||
                    null);
            }
            return schema;
        }
        ...
        function generateExampleFromSchema(schema, depth) {
            const currentDepth = typeof depth === 'number' ? depth : 0;
            if (!schema || currentDepth > 4)
                return undefined;
            ...
            if (resolved.oneOf && Array.isArray(resolved.oneOf) && resolved.oneOf.length > 0) {
                return generateExampleFromSchema(resolved.oneOf[0], currentDepth + 1);
            }
            if (resolved.anyOf && Array.isArray(resolved.anyOf) && resolved.anyOf.length > 0) {
                return generateExampleFromSchema(resolved.anyOf[0], currentDepth + 1);
            }
            if (resolved.allOf && Array.isArray(resolved.allOf) && resolved.allOf.length > 0) {
                const merged = {};
                resolved.allOf.forEach((part) => {
                    const value = generateExampleFromSchema(part, currentDepth + 1);
            ...
            if (resolved.type === 'array' || resolved.items) {
                const itemExample = generateExampleFromSchema(resolved.items, currentDepth + 1);
            ...
            if (resolved.type === 'object' || resolved.properties) {
                ...
                Object.entries(props).forEach(([key, propSchema]) => {
                    const value = generateExampleFromSchema(propSchema, currentDepth + 1);
        ...
        function getSuccessExamples(jsonResp) {
            ...
            if (schema) {
                const generated = generateExampleFromSchema(schema, 0);
        ...
        function extractErrorSummary(resp) {
            ...
            for (const example of examples) {
                if (!example || typeof example !== 'object')
                    continue;
                if ('errorCode' in example && example.errorCode != null && errorCode === '—') {
                    errorCode = String(example.errorCode);
                }
                if ('message' in example && example.message != null) {
        ...
        function collectEnumDefinitionsFromSchema(schema, prefix, visited) {
            ...
            if (!resolved || typeof resolved !== 'object')
                return [];
            const results = [];
            const visitKey = currentPrefix + '|' + (resolved.$ref || '');
            if (currentVisited.has(visitKey))
                return results;
            currentVisited.add(visitKey);
            if (Array.isArray(resolved.enum) && resolved.enum.length > 0) {
                results.push({
                    title: currentPrefix || 'body',
            ...
            if (props) {
                Object.entries(props).forEach(([name, propSchema]) => {
                    const fieldPath = currentPrefix ? currentPrefix + '.' + name : name;
                    results.push(...collectEnumDefinitionsFromSchema(propSchema, fieldPath, currentVisited));
                });
            }
            if (resolved.items) {
                const itemPath = currentPrefix ? currentPrefix + '[]' : 'items[]';
                results.push(...collectEnumDefinitionsFromSchema(resolved.items, itemPath, currentVisited));
            }
            ['allOf', 'oneOf', 'anyOf'].forEach((key) => {
                ...
                if (!Array.isArray(list))
                    return;
                list.forEach((part) => {
                    results.push(...collectEnumDefinitionsFromSchema(part, currentPrefix, currentVisited));
                });
            });
        ...
        function renderEnumDefinitionsSection(enumDefs, title) {
            if (!Array.isArray(enumDefs) || enumDefs.length === 0)
                return '';
            let html = '<section class="enum-definitions-section"><h3>' +
                escapeHtml(title) +
                '</h3>';
        ...
        function renderSchemaRows(schema, prefix, depth, visited) {
            const currentPrefix = prefix || '';
            const currentDepth = typeof depth === 'number' ? depth : 0;
            const currentVisited = visited || new Set();
            const resolvedSchema = resolveSchema(schema) || schema;
            if (!resolvedSchema || !resolvedSchema.properties || currentDepth > 4)
                return '';
            const visitKey = currentPrefix + '|' + currentDepth;
            if (currentVisited.has(visitKey))
                return '';
            currentVisited.add(visitKey);
            let html = '';
            ...
            for (const [name, prop] of Object.entries(resolvedSchema.properties)) {
                ...
                if (parsed.type === 'object' || parsed.properties) {
                    html += renderSchemaRows(parsed, fullName, currentDepth + 1, currentVisited);
                    continue;
                }
                if (parsed.type === 'array' && parsed.items) {
                    const itemSchema = resolveSchema(parsed.items) || parsed.items;
                    if (itemSchema && (itemSchema.type === 'object' || itemSchema.properties)) {
                        html += renderSchemaRows(itemSchema, fullName + '[]', currentDepth + 1, currentVisited);
                    }
                }
            }
            return html;
        ...
        function resolveResponseSchemaForTable(schema) {
            if (!schema)
                return null;
            const parsed = resolveSchema(schema) || schema;
        ...
        function renderSuccessExamplesBlock(successExamples) {
        ...
        function buildEndpointDoc(ep, tag) {
            ...
            md +=
                '<span class="endpoint-method-badge method-' +
                    ep.method +
                    '">' +
                    ep.method +
                    '</span>';
            ...
            const routeParams = (ep.parameters || []).filter((param) => param && (param.in === 'query' || param.in === 'path'));
            ...
            if (routeParams.length > 0) {
                ...
                md +=
                    '<p>Não é necessário enviar parâmetros nesta requisição.</p>';
            }
            ...
            if (ep.requestBody) {
                ...
                if (content && content['application/json']) {
                    ...
                    md += renderEnumDefinitionsSection(collectEnumDefinitionsFromSchema(schema, '', new Set()), 'Enums do Corpo da Requisição');
                    ...
                    md +=
                        '<p>Esta requisição não possui corpo em <code>application/json</code>.</p>';
                }
            }
            else {
            ...
            if (ep.responses) {
                ...
                if (errorResponses.length > 0) {
                    md += '<h2>Erros</h2>';
                    md +=
                        '<p>Este endpoint pode retornar erros específicos, conforme a tabela a seguir:</p>';

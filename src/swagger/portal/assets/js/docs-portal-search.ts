(function initDocsPortalSearchModule(globalScope) {
  ...
  interface SearchDeps {
    ...
    getEndpointsByTag: () => Record<string, EndpointDoc[]>;
    ...
    stripMd: (value: string) => string;
    getSearchSelectedIndex: () => number;
    setSearchSelectedIndex: (value: number) => void;
    getAllSearchItems: () => SearchItem[];
    setAllSearchItems: (items: SearchItem[]) => void;
  }

  function createSearchModule(deps: SearchDeps) {
    const {
      getManualTags,
      getEndpointsByTag,
      getActiveView,
      ...
      for (const [tag, endpoints] of Object.entries(getEndpointsByTag())) {
        endpoints.forEach((endpoint, index) => {
          items.push({
            ...
            action: () => {
              if (getActiveView() !== 'manual') switchView('manual');
    ...
    function onSearchInput(query: string) {
      ...
      if (!items.length) {
        results.innerHTML =
          '<div class="search-empty">Nenhum resultado para "' + escapeHtml(query) + '"</div>';
        return;
      }
    ...
    function bindKeyboardShortcuts() {
      document.addEventListener('keydown', (event: KeyboardEvent) => {
        ...
        if (event.key === 'Enter') {
          event.preventDefault();
          const selected = items[getSearchSelectedIndex()] as HTMLElement | undefined;
          if (selected && typeof selected.click === 'function') selected.click();
        }
      });
    }

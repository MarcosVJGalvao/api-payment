(function initDocsPortalSidebarModule(globalScope) {
    function createSidebarModule(deps) {
        const { getManualTags, getEndpointsByTag, escapeHtml, marked, slugifyManualName, setManualUrl, setEndpointUrl, closeManualSidebar, renderManualHtml, buildEndpointDoc, setCurrentEndpointSelection, setCurrentManualSelection, getCurrentManualSelection, } = deps;
        ...
        function selectEndpoint(tag, index) {
            ...
            groups.forEach((group) => {
                ...
                if (headerText === tag) {
                    ...
                    if (endpoints[index + offset]) {
                        endpoints[index + offset].classList.add('active');
                    }
                }
            });
            const endpointsByTag = getEndpointsByTag();
            const endpoint = endpointsByTag[tag]?.[index];
            if (!endpoint)
                return;
        ...
        function selectGroupOverview(tag) {
            ...
            content.querySelectorAll('.endpoint-row').forEach((row) => {
                row.addEventListener('click', () => {
                    ...
                    const rowIndex = Number.parseInt(row.dataset.idx || '-1', 10);
                    if (!rowTag || Number.isNaN(rowIndex) || rowIndex < 0)
                        return;
                    selectEndpoint(rowTag, rowIndex);
                });
            });
        }
        function groupTagsByAuth(authResolver) {
            const authGroups = {};
            Object.entries(getEndpointsByTag()).forEach(([tag, endpoints]) => {
                const authKey = authResolver.getPrimaryAuthKey(endpoints[0]);
                if (!authGroups[authKey])
                    authGroups[authKey] = [];
        ...
        function renderApiGroups(authResolver) {
            const sidebar = document.getElementById('manual-sidebar');
            if (!sidebar)
                return;
            const title = document.createElement('div');
            ...
            groupTagsByAuth(authResolver).forEach((authBlock) => {
                ...
                authBlock.groups.forEach(({ tag, endpoints }) => {
                    ...
                    header.innerHTML =
                        '<span>' + escapeHtml(tag) + '</span><span class="arrow">▶</span>';
        ...
        function ensureManualSelection() {
            ...
            if (selection && selection.type === 'manual' && Number.isInteger(selection.index)) {
                selectManualItem(selection.index);
            }
        }
        return {
            clearActive,

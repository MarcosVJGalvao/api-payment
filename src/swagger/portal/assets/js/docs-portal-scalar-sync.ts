(function initDocsPortalScalarSyncModule(globalScope) {
    function createScalarSyncModule(deps) {
        const { getScalarUrl, getEndpointsByTag, slugifyEndpointTag, getCurrentEndpointSelection, getActiveView, getScalarLoaded, setScalarLoaded, } = deps;
        function getEndpointByTagAndIndex(tag, index) {
            ...
            if (!Array.isArray(endpoints))
                return null;
            if (!Number.isInteger(index) || index < 0 || index >= endpoints.length)
                return null;
            return endpoints[index];
        }
        function getScalarRefFromPortalHash() {
            const hash = window.location.hash || '';
            if (!hash.startsWith('#scalarRef='))
                return null;
        ...
        function applyScalarEndpointSelectionToIframe() {
            ...
            if (!iframe)
                return;
            const hash = buildScalarHashForEndpoint(currentEndpointSelection.tag, currentEndpointSelection.index);
            if (!hash)
                return;
            if (!getScalarLoaded()) {
            ...
            try {
                ...
                const tryFindByMethodAndPath = (method, normalizedPath, preferredTagSlug = null) => {
                    ...
                    for (const [tag, endpoints] of Object.entries(endpointsByTag || {})) {
                        ...
                        const index = (endpoints || []).findIndex((ep) => String(ep.method || '').toUpperCase() === upperMethod &&
                ...
                const tryExtractVisibleOperationHeader = (iframeDoc) => {
                    const root = iframeDoc?.querySelector('[role="main"]') ||
                    ...
                    for (const el of elements) {
                        if (!(el instanceof iframe.contentWindow.HTMLElement))
                            continue;
                        if (!el.offsetParent)
                            continue;
                        ...
                        if (rect.width < 120 || rect.height < 10)
                            continue;
                        if (rect.bottom < 0 || rect.top > (iframe.contentWindow.innerHeight || 10000))
                            continue;
                        ...
                        const score = (inRightPane ? 1000 : 0) -
                ...
                const tryExtractFromVisibleCodeSnippet = (iframeDoc) => {
                    ...
                    for (const el of blocks) {
                        ...
                        if (!text)
                            continue;
                        if (!/\bmethod\s*:/.test(text) || !/\burl\s*:/.test(text))
                            continue;
                ...
                if (codeSnippetSelection)
                    return codeSnippetSelection;
                const visibleHeaderSelection = tryExtractVisibleOperationHeader(iframeDoc);
                if (visibleHeaderSelection)
                    return visibleHeaderSelection;
                const focusedText = iframeDoc?.querySelector('main')?.innerText ||
                ...
                while ((urlMatch = urlPattern.exec(focusedText)) != null) {
                    ...
                    const localMethodMatches = Array.from(localChunk.matchAll(/\bmethod\s*:\s*['"]?(GET|POST|PUT|PATCH|DELETE)['"]?/gi));
                    const localMethods = localMethodMatches.map((m) => m[1].toUpperCase());
                    const methodHints = localMethods.length
        ...
        function syncApiPortalUrlFromScalarSelection() {
            ...
            if (!selection)
                return;
            const nextPath = '/docs/api/endpoint/' +

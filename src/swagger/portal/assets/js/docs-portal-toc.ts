(function initDocsPortalTocModule(globalScope) {
  ...
  interface TocDeps {
    ...
    getManualTocItems: () => TocItem[];
    setManualTocItems: (items: TocItem[]) => void;
    escapeHtml: (value: string) => string;
    slugifyHeading: (value: string) => string;
  }

  function createTocModule(deps: TocDeps) {
    const {
      getActiveView,
      getManualTocItems,
      setManualTocItems,
    ...
    function updateManualTocActive() {
      ...
      manualTocItems.forEach((item, itemIndex) => {
        ...
        if (rect.top <= anchorLine) activeIndex = itemIndex;
      });
    ...
    function bindManualTocScrollSpy() {
      ...
      content.addEventListener('scroll', () => {
        if (getActiveView() === 'manual') updateManualTocActive();
      });
    ...
    function decorateHeadingAnchors() {
      ...
      headings.forEach((heading, index) => {
        ...
        if (!headingEl.id) {
          headingEl.id = slugifyHeading(text) || 'section-' + index;
        }

        if (headingEl.querySelector('.heading-anchor-link')) return;

        const anchor = document.createElement('a');
        anchor.className = 'heading-anchor-link';
        anchor.href = '#' + headingEl.id;
        ...
        anchor.addEventListener('click', (event: MouseEvent) => {
          ...
          const hash = '#' + headingEl.id;
          const nextUrl = window.location.pathname + (window.location.search || '') + hash;
          window.history.replaceState({}, '', nextUrl);
          headingEl.scrollIntoView({ behavior: 'smooth', block: 'start' });
        });

        headingEl.classList.add('has-heading-anchor');
        headingEl.insertBefore(anchor, headingEl.firstChild);
      });
    }
    ...
    function buildManualToc() {
      ...
      const tocCandidates: TocItem[] = [];
      const usedIds = new Set<string>();
      let firstLevel: number | null = null;
      let secondLevel: number | null = null;
      ...
      headings.forEach((element, index) => {
        ...
        if (!text) return;
        if (element.classList.contains('endpoint-path')) return;
        if (element.closest('.enum-definitions-section')) return;
        ...
        if (rawLevel > firstLevel) {
          if (secondLevel === null) secondLevel = rawLevel;
          if (rawLevel !== secondLevel) return;
      ...
      tocList.innerHTML = manualTocItems
        .map(
          (item, index) =>
            '<button type="button" class="manual-toc-item level-' +
            item.level +
            '" data-toc-index="' +
            index +
            '" title="' +
      ...
      tocList.querySelectorAll('.manual-toc-item').forEach((button) => {
        (button as HTMLElement).addEventListener('click', () => {
          const indexValue = Number((button as HTMLElement).dataset.tocIndex);
          const selectedItem = getManualTocItems()[indexValue];
          if (!selectedItem) return;
          selectedItem.el.scrollIntoView({ behavior: 'smooth', block: 'start' });
          setActiveTocItem(indexValue);
        });
      });
    ...
    function renderManualHtml(html: string) {
      const content = document.getElementById('manual-content');
      if (!content) return;

...
import {
  mockBackofficeLoginDto,
  mockBankLoginDto,
} from './mocks/hiperbanco-dto.mock';
...
describe('HiperbancoAuthService', () => {
  let service: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  >['service'];
  let httpServiceMock: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  >['httpServiceMock'];
  let credentialsServiceMock: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  >['credentialsServiceMock'];
  let sessionServiceMock: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  >['sessionServiceMock'];
  let jwtServiceMock: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  >['jwtServiceMock'];
  let accountServiceMock: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  >['accountServiceMock'];
  let onboardingServiceMock: Awaited<
    ReturnType<typeof createHiperbancoAuthTestFactory>
  ...
  describe('loginBackoffice', () => {
    it('deve autenticar e retornar token JWT com sessionId', async () => {
      ...
      httpServiceMock.post.mockResolvedValue({
        access_token: 'hiperbanco-token',
      });

      const result = await service.loginBackoffice(loginDto);

      expect(credentialsServiceMock.getPublicCredentials).toHaveBeenCalledWith(
        FinancialProvider.HIPERBANCO,
        ProviderLoginType.BACKOFFICE,
      );
      expect(httpServiceMock.post).toHaveBeenCalledWith('/Backoffice/Login', {
  ...
  describe('loginApiBank', () => {
    it('deve autenticar usuário e retornar token JWT com sessionId', async () => {
      ...
      onboardingServiceMock.createOrUpdate.mockResolvedValue(

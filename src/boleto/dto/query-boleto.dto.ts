...
export class QueryBoletoDto extends BaseQueryDto {
    @ApiPropertyOptional({ enum: BoletoStatus, description: 'Filtrar por status do boleto' })
    @IsOptional()
    @IsEnum(BoletoStatus)
    status?: BoletoStatus;

    @ApiPropertyOptional({ enum: BoletoType, description: 'Filtrar por tipo de boleto' })
    @IsOptional()
    @IsEnum(BoletoType)
    type?: BoletoType;

    @ApiPropertyOptional({ enum: FinancialProvider, description: 'Filtrar por provedor financeiro' })
    @IsOptional()

...
export class BoletoSyncHelper {
     ...
     */
    async syncBoletoWithProvider(boleto: Boleto, session: ProviderSession): Promise<Boleto> {
        // Atualmente sincronizamos apenas boletos do Hiperbanco que possuem authenticationCode
        if (boleto.providerSlug !== FinancialProvider.HIPERBANCO || !boleto.authenticationCode) {
            return boleto;
        }

        try {
            this.logger.log(`Fetching updated boleto data from Hiperbanco for boleto: ${boleto.id}`, this.context);

            const boletoData: BoletoGetDataResponse = await this.providerHelper.getBoletoData(
                boleto.providerSlug,
            ...
            if (Object.keys(updateData).length > 0) {
                ...
                const updatedBoleto = await this.repository.findOne({ where: { id: boleto.id } });
                if (updatedBoleto) {
                    this.logger.log(`Boleto data persisted from Hiperbanco: ${boleto.id}`, this.context);
